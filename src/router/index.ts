...
import LuFeiRulesPage from '@/views/LuFeiRulesPage.vue'
...
const childrenRouter = [
  ...
  {
    path: 'lufei-rules',
    name: ROUTE_NAME.lufeiRules,
    component: LuFeiRulesPage,
  },
  {
    path: 'settings',
...
const title = useTitle('Lufei-ClashBoard')
const setTitleByName = (name: string | symbol | undefined) => {
  if (typeof name === 'string' && activeBackend.value) {
    title.value = `Lufei-ClashBoard | ${i18n.global.t(name)}`
  } else {
    title.value = 'Lufei-ClashBoard'
  }
}
