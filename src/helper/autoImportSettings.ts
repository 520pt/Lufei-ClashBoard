...
const LUFEI_PROXY_GROUP_ICON_NAME = '路飞'
const LUFEI_PROXY_GROUP_ICON_UUID = '8f949d69-cd69-4773-b62f-61aeb5150cf2'
const LUFEI_PROXY_GROUP_ICON =
...
const ensureLufeiProxyGroupIcon = (settings: Record<string, unknown>) => {
  ...
  const nextIconReflectList = iconReflectList
    .filter((item): item is { name: string; icon: string; uuid: string } => {
      return Boolean(
        ...
        typeof item === 'object' &&
        'name' in item &&
        item.name !== LUFEI_PROXY_GROUP_ICON_NAME,
      )
    })
    .concat({
      name: LUFEI_PROXY_GROUP_ICON_NAME,
      icon: LUFEI_PROXY_GROUP_ICON,
      uuid: LUFEI_PROXY_GROUP_ICON_UUID,
    })
...
const sanitizeLufeiSelfUseSettings = (settings: Record<string, unknown>) => {
  ...
  return ensureLufeiProxyGroupIcon(sanitized)
}
